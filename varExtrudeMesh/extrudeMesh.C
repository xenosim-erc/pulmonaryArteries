...
int main(int argc, char *argv[])
{
    ...
    if (mode == PATCH || mode == MESH)
    {
        ...
        // Read the requested total extrusion distance at every point of the
        // source volume mesh.  The field name is selected in
        // varExtrudeMeshDict, for example "distanceField WallThickness;".
        // The mapper writes this as a pointScalarField in the source case.
        const word distanceFieldName(dict.get<word>("distanceField"));
        const pointMesh& sourcePointMesh = pointMesh::New(mesh);
        const pointScalarField extrusionDistance
        (
            IOobject
            (
                distanceFieldName,
        ...
        if (extrusionDistance.dimensions() != dimLength)
        {
            FatalErrorInFunction
                << "Point field " << distanceFieldName << " has dimensions "
                << extrusionDistance.dimensions() << ", but an extrusion "
                << "distance must have length dimensions " << dimLength
                << exit(FatalError);
        }

        // Convert from global source-mesh point numbering to the local point
        // ordering of the selected extrusion patch.  This guarantees that
        // patchDistance[pointi], localPoints()[pointi], and the calculated
        // patch normal all describe the same physical point.
        scalarField patchDistance(extrudePatch.nPoints(), Zero);
        const labelList& patchMeshPoints = extrudePatch.meshPoints();

        forAll(patchDistance, pointi)
        {
            patchDistance[pointi] =
                extrusionDistance[patchMeshPoints[pointi]];

            if (patchDistance[pointi] <= VSMALL)
            {
                FatalErrorInFunction
                    << "Non-positive extrusion distance "
                    << patchDistance[pointi] << " at patch point " << pointi
                    << " (mesh point " << patchMeshPoints[pointi] << ")"
                    << exit(FatalError);
            }
        }

        Info<< "Using point extrusion-distance field " << distanceFieldName
            << nl << "    minimum distance: " << gMin(patchDistance)
            << nl << "    maximum distance: " << gMax(patchDistance)
            << nl << "    average distance: " << gAverage(patchDistance)
            << nl << endl;
        ...
        forAll(displacement, pointi)
        {
            ...
            // Preserve the source patch exactly as layer 0.  This is the
            // conformal fluid-solid interface.
            layer0Points[pointi] = extrudePatch.localPoints()[pointi];

            // displacement is the complete layer-0 to layer-N movement.
            // addPatchCellLayer uses the first-layer fraction below, while
            // the final point-reset loop assigns every layer explicitly.
            displacement[pointi] = patchDistance[pointi]*patchNormal;
        }
        ...
        forAll(layerExtrude.addedPoints(), pointi)
        {
            ...
            forAll(pPoints, pPointi)
            {
                ...
                const label layer = pPointi + 1;
                const point modelPt
                (
                    extrudePatch.localPoints()[pointi]
                  + model().sumThickness(layer)
                   *patchDistance[pointi]
